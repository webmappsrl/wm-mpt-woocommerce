...
/**
 * Sets the last 3 orders order_paid_date to X days from now if e specific order_id is not given as second parameter. sets the expiry date to the day before if parameter 0 is given. Accepts one numeric arguments as days and the second as order ID. 
 *
 *
 * @when after_wp_load
 */
$wm_mpt_set_order_paid_date = function( $args, $assoc_args )
{
    ...
    if ($days && is_int($days) && $days > 0) {
        ...
        if ($custom_order) {
            ...
            $today = date('Y-m-d');
            $current_paid_date = get_field('order_paid_date',$order->ID);
            $new_date = date("Y-m-d", strtotime("-1 years -1 day", strtotime($today)));
            update_field('order_paid_date', $new_date, $order->ID);
